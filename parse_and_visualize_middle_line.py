...
def calculate_area_of_one_left_lane(left_lane, points, most_left_points):
    ...
    for reference_point, inner_point in zip(points, inner_points):
        ...
        x_middle = x_inner + cos(normal_left) * (lane_width_offset / 2)
        y_middle = y_inner + sin(normal_left) * (lane_width_offset / 2)

        update_points.append((x_outer, y_outer))
        update_middle_points.append((x_middle, y_middle))

    outer_points = update_points[:]
    most_left_points = outer_points[:]

    middle_points = update_middle_points[:]
    most_middle_points = middle_points[:]
    # 这个地方需要删除中心线上x和y为弧线的部分
    current_ara = {
        "inner": inner_points,
        "outer": outer_points,
...
def calculate_area_of_one_right_lane(right_lane, points, most_right_points):
    ...
    for reference_point, inner_point in zip(points, inner_points):
        ...
        x_middle = x_inner + cos(normal_eight) * (lane_width_offset / 2)
        y_middle = y_inner + sin(normal_eight) * (lane_width_offset / 2)
...
def plot_planes_of_roads(total_areas, save_folder):
    ...
    # 先清空这个scr文件
    open_file = open('D:\\my_github_learn\\data_test\\line.scr', mode='w')
    # 再重新写这个文件。
    open_file = open('D:\\my_github_learn\\data_test\\line.scr', mode='a')
    ...
    for k, v in tqdm(total_areas.items(), desc="Ploting Edges"):
        ...
        for left_lane_id, left_lane_area in left_lanes_area.items():
            ...
            inner_points = left_lane_area["inner"]
            outer_points = left_lane_area["outer"]
            middle_points = left_lane_area["middle"]
            points_of_one_road = inner_points + outer_points[::-1]
            xs = [i for i, _ in points_of_one_road]
            ys = [i for _, i in points_of_one_road]
            xs_inner = [i for i, _ in inner_points]
            ys_inner = [i for _, i in inner_points]
            xs_outer = [i for i, _ in outer_points]
            ys_outer = [i for _, i in outer_points]
            xs_middle = [i for i, _ in middle_points]
            ys_middle = [i for _, i in middle_points]
            # plt.plot(xs, ys)
            plt.plot(xs_inner, ys_inner, color="black")
            plt.plot(xs_outer, ys_outer, color="black")
            plt.plot(xs_middle, ys_middle, color="red", linestyle='--')
            # for i in range(len(xs_middle)-1):
            #     open_file.write(f"line {xs_middle[i]},{ys_middle[i]} {xs_middle[i+1]},{ys_middle[i+1]} \n")
            open_file.write(f"line {xs_middle[0]},{ys_middle[0]} {xs_middle[len(xs_middle)-1]},{ys_middle[len(xs_middle)-1]} \n")
            #plt.show()
            # plt.scatter(xs[::area_select], ys[::area_select], color=rescale_color(lane_color, 0.5), s=1)
        ...
        for right_lane_id, right_lane_area in right_lanes_area.items():
            ...
            inner_points = right_lane_area["inner"]
            outer_points = right_lane_area["outer"]
            middle_points = right_lane_area["middle"]
            points_of_one_road = inner_points + outer_points[::-1]
            xs = [i for i, _ in points_of_one_road]
            ys = [i for _, i in points_of_one_road]
            xs_inner = [i for i, _ in inner_points]
...
def process_one_file(file, step=0.1):
    ...
    #把车道线数据保存到文件中
    #save_lane_data_to_file(total_areas, save_folder)

def save_lane_data_to_file(total_areas, save_folder):
    """
    ...
    with open(save_file, "w") as file:
        file.write("lane_id,start_point_x,start_point_y,end_point_x,end_point_y\n")
        for k, v in total_areas.items():
            ...
            for left_lane_id, left_lane_area in left_lanes_area.items():
                ...
                xs_middle = [i for i, _ in middle_points]
                ys_middle = [i for _, i in middle_points]
                file.write(f"{k[0],left_lane_id,xs_middle[0]},{ys_middle[0]},{xs_middle[len(xs_middle) - 1]},{ys_middle[len(xs_middle) - 1]}\n")
            for right_lane_id, right_lane_area in right_lanes_area.items():
                middle_points = right_lane_area["middle"]
                xs_middle = [i for i, _ in middle_points]
                ys_middle = [i for _, i in middle_points]
                file.write(f"{k[0],right_lane_id,xs_middle[0]},{ys_middle[0]},{xs_middle[len(xs_middle) - 1]},{ys_middle[len(xs_middle) - 1]}\n")
    file.flush()
    file.close()
